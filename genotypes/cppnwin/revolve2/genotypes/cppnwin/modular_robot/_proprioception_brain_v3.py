...
class ProprioceptionCPPNNetwork(ActorController):
    ...
    def develop_controller_ES(self, exp=None):
        if not hasattr(self, 'brain_net'):from typing import List, Set, Tuple, cast
import random
import multineat
from squaternion import Quaternion
import pprint
...
class ProprioceptionCPPNNetwork(ActorController):
    ...
    def develop_controller_ES(self, exp=None):
        ...
        self.controller.develop_from_cppn(cppn=self.brain_net,
                                          substrate_dimensions=substrate_dimensions,
                                          num_inputs=self._n_joints,
                                          num_outputs=self._n_joints,
                                          connection_threshold=0.2)

        self.plot_nn(self.controller, exp=exp)
    ...
    def step(self, dt: float, exp = None) -> None:
        ...
        self.develop_controller_ES(exp)
    ...
    def plot_nn(self, nn, exp = None):
        ...
        if not os.path.exists(str(exp)+'/nn_plots'):
            os.makedirs(str(exp)+'/nn_plots')
        import time
        plt.savefig(str(exp)+'/nn_plots/nn_plot'+str(time.time())+'.png')
        # plt.show()
        plt.close()
        self.brain_net = multineat.NeuralNetwork()
        self._genotype.BuildPhenotype(self.brain_net)
